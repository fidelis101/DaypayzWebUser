...
{
    private $vtl,$vtpass,$clubkonnect,$mobileairtime,$airvend;
...
//            }
        ...
        if($result || $result == true)
		{
            ...
            TransactionHistoryHandler::saveTranHistoryTran(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$amount,'Receiver'=>$mobilenumber,
            'Sender'=>$username,'Description'=>'Debit: You Recharged '.$networkName.' '.$mobilenumber,'Request_Id'=>$request_id,'Tran_Id'=>$tran_id,'Status'=>$status_id,
        ...
        else
        {
            ...
            'Sender'=>'Recharge','Description'=>'Refund: Airtime Recharge '.$networkName.' '.$mobilenumber.' failed','Request_Id'=>$request_id,'Tran_Id'=>$tran_id,
            ...
            if(strlen($amount) < 2)
                {
                    ...
                    if(strpos($amount,"0.5SME"))
                    {
                        $result = $this->digitalvendorz->data($network,$amount,$mobilenumber,$username,$cost,$request_id);
...
//            }
        ...
        if($result)
        {
            ...
            $tranHis = mysqli_query($con,"SELECT Id FROM transactionhistory WHERE Request_Id='$initial_id'");
    ...
    public function subCable($invoiceperiod,$cable,$amount,$customernum,$customernumber,$customername,$smartcardno,$username,$cost,$balance,$newbalance)
    {
        ...
        else
        {
            ...
            // if(!$result->isSuccessful && $amount != "00" && $amount != "05")
            // {
            //     $requestid = uniqid();
            //     $result = $this->vtpass->cable($invoiceperiod,$cable,$amount,$customernum,$customernumber,$customername,$smartcardno,$username,$cost,$balance,$newbalance,$requestid);
            // }
        ...
        if($result->isSuccessful)
        {
            ...
            TransactionHistoryHandler::saveTranHistoryTran(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$cost,'Receiver'=>$smartcardno,
        ...
        {    
            ...
            TransactionHistoryHandler::saveTranHistoryTran(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$cost,'Receiver'=>$username,
...
public function subCable_d($invoiceperiod,$cable,$amount,$customernum,$customernumber,$customername,$smartcardno,$username,$cost,$balance,$newbalance)
     {
        include('dbcon.php');
    ...
    public function subDisco($provider,$package,$amount,$customernumber,$meterno,$username,$balance,$newbalance)
    {
        ...
        $requestid = uniqid();
        $dicoMap = ["01"=>"Eko","02"=>"Ikeja","03"=>"Ibadan","04"=>"Abuja","05"=>"Portharcourt","06"=>"Kano","07"=>"EEDC","08"=>"Jos", "09"=>"Benin"];
        $discoProvider = $dicoMap[$provider];
        $providerCode =$provider;
        
        if($provider == "05" || $provider == "06" || $provider == "08" || $provider == "09")
        {
            $result = $this->vtpass->disco($provider,$package,$amount,$customernumber,$meterno,$username,$balance,$newbalance,$requestid);
        }
        else if($provider == "01" || $provider == "02" || $provider == "03" || $provider == "04" || $provider == "07")
        {
            if($provider != "02" || $provider == "04")
        ...
        if($result->isSuccessful)
		{
           ...
           //$description = 'Debit: You Subscribed '.$dicoMap[$provider].' meter  '.$meterno;
            ...
            TransactionHistoryHandler::saveTranHistoryTran(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$amount,'Receiver'=>$meterno,
    ...
    public function BuyEducation($pintype,$pins,$cost,$username,$newbalance,$email,$balance)
    {
        ...
        if($result->isSuccessful)
        {
           ...
           //$description = 'Debit: You Subscribed '.$dicoMap[$provider].' meter  '.$meterno;
            ...
            TransactionHistoryHandler::saveTranHistoryTran(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$cost,'Receiver'=>$eduTypes[$pintype],
	...
	function creditAirtimeBonus($username,$amount,$number,$network)
	{
		...
		if(mysqli_num_rows($network1)>0)
		{
			...
			if($result->num_rows > 0)
			{
                ...
                TransactionHistoryHandler::saveTransactionHistory(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$amount*$bonusR,'Receiver'=>$referal,
			...
			if($result->num_rows > 0)
			{
				...
				mysqli_query($con,"UPDATE wallets SET BonusBalance='$bal' WHERE Username='$referal'");
                TransactionHistoryHandler::saveTransactionHistory(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$amount*$bonusR1,'Receiver'=>$referal,
            ...
            while($i <= 5)
            {
				...
				}else
				{
                    ...
                    mysqli_query($con,"UPDATE wallets SET BonusBalance='$bal' WHERE Username='$referal'");
                    TransactionHistoryHandler::saveTransactionHistory(array('TransactionDate'=>date("Y-m-d H:i:s"),'Amount'=>$amount*$bonusR2,'Receiver'=>$referal,
