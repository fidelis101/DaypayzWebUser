...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	$str = preg_replace_callback('/\&\#([0-9]+)\;/', function($found){ return strtolower($found[1]);},$str);
	$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/', function($found){ return strtolower($found[1]);},$str);
	return $str;
  }
}
